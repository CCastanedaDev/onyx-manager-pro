...
class VoidWindow(ctk.CTk):
    def __init__(self):
        ...
        # --- NUEVO: Sincronización Inicial con Archivos Reales ---
    ...
    def accion_detener(self):
        ...
        # 3. Actualizar UI inmediatamente (Sin esperar)
        # Asumimos que el servidor nos hará caso y echará a los jugadores.
        ...
        self.log_sistema("✅ Servidor desconectado. (Jugadores siendo expulsados...)")
    ...
    def crear_sidebar(self):
        ...
        self.crear_boton_menu(self.lang.get("menu_raid"), "raid", 8)
        self.crear_boton_menu(self.lang.get("menu_explorer"), "explorer", 9)
        self.crear_boton_menu(self.lang.get("menu_logs"), "logs", 10)
        ctk.CTkLabel(self.sidebar, text="v1.0 PRO", text_color=COLORS["text_dim"]).grid(row=11, column=0, pady=20, sticky="s")
    ...
    def sincronizar_todo_con_archivos(self):
        ...
        self.log_sistema("🔄 Sincronizando con archivos originales del juego...")
        
        # 1. Configuración del Servidor (.ini)
        datos_ini = self.editor.cargar_configuracion()
        if datos_ini:
            def rellenar_if_exists(entry, key):
                if key in datos_ini:
                    entry.delete(0, "end")
                    entry.insert(0, datos_ini[key])

            rellenar_if_exists(self.entry_name, "scum.ServerName")
            rellenar_if_exists(self.entry_desc, "scum.ServerDescription")
            rellenar_if_exists(self.entry_motd, "scum.MessageOfTheDay")
            rellenar_if_exists(self.entry_pass, "scum.ServerPassword")
            rellenar_if_exists(self.entry_players, "scum.MaxPlayers")
    ...
    def cerrar_aplicacion(self):
        ...
        # 2. Detener servidor si está corriendo (Instantáneo)
        # Esto cierra el pipe stdin, enviando la señal de apagado a SCUM.
        if self.steam.esta_corriendo():
            self.steam.detener_servidor() 
            
        # 3. Forzar salida inmediata
        # Esto evita que hilos "zombies" cuelguen la UI y asegura que el OS cierre los pipes.
        self.destroy()
        os._exit(0)
    ...
    def construir_scheduler(self):
        ...
        self.combo_perfiles_lib = ctk.CTkComboBox(frame_top, values=list(self.scheduler.perfiles.keys()), width=200, command=self.evento_cambio_perfil)
    ...
    def evento_cambio_perfil(self, choice):
        self.accion_cargar_mesa_trabajo()
    ...
    def construir_tasks(self):
        ...
        for i, dia in enumerate(dias_semana):
            ...
            ctk.CTkCheckBox(row_days, text=dias_labels[i], variable=var, width=50, checkbox_width=20, checkbox_height=20).pack(side="left", padx=5)
    ...
    def actualizar_jugadores_a2s(self):
        """Consulta directa al servidor usando protocolo Source (A2S)"""
        try:
            # SIEMPRE usar localhost para la consulta interna (evita problemas de NAT Loopback)
            # El puerto sí lo sacamos de la UI
            ip = "127.0.0.1"
            
            q_port = self.entry_query.get().strip()
            if not q_port.isdigit(): q_port = 27015
            else: q_port = int(q_port)

            # Consulta A2S
            address = (ip, q_port)
            info = a2s.info(address, timeout=1.0)
            
            # Actualizar Label con datos reales
            self.lbl_players.configure(text=f"{self.lang.get('label_players')} {info.player_count}/{info.max_players}", text_color=COLORS["success"])
            
            # Sincronizar contador interno por si acaso
            self.player_count_log = info.player_count
            
        except Exception as e:
            # FALLBACK: BATTLEMETRICS (Si A2S falla)
            # Ya no usamos logs porque no era fiable.
            threading.Thread(target=self.actualizar_jugadores_battlemetrics).start()

    def actualizar_jugadores_battlemetrics(self):
        try:
            server_name = self.entry_name.get().strip()
            if not server_name: return

            # 1. Buscar ID si no lo tenemos
            if not self.battlemetrics_server_id:
                url_search = f"https://api.battlemetrics.com/servers?filter[search]={server_name}&filter[game]=scum"
                ...
                if r.status_code == 200:
                    ...
                    if data["data"]:
                        ...
                        self.battlemetrics_server_id = data["data"][0]["id"]
            
            # 2. Consultar info con ID
            if self.battlemetrics_server_id:
                url_info = f"https://api.battlemetrics.com/servers/{self.battlemetrics_server_id}"
                headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"}
                r = requests.get(url_info, headers=headers, timeout=5)
                if r.status_code == 200:
                    data = r.json()
                    ...
                    # Actualizar UI (Sobrescribe el (Log) si esto funciona)
                    self.lbl_players.configure(text=f"{self.lang.get('label_players')} {players}/{max_players} (BM)", text_color="#3498DB")
        except: pass
