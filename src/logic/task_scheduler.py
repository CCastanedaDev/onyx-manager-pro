    ...
    def ejecutar_tarea(self, nombre_perfil):
        ...
        try:
            if nombre_perfil == "BACKUP":
                ...
                self.log(f">> [AUTO] Iniciando aplicación de perfil: {nombre_perfil}")
                contenido = self.perfiles[nombre_perfil]
                
                # 1. Detener el servidor
                self.steam.detener_servidor()
                
                # 2. Espera de seguridad inteligente (Smart Wait Loop)
                self.log("⏳ Esperando cierre completo del servidor para aplicar perfil...")
                stopped = False
                for i in range(65): # Un poco más que 60 para margen
                    if not self.steam.esta_corriendo():
                        stopped = True
                        break
                    if i == 60:
                        self.log("⚠️ El servidor no cierra. Forzando cierre (Taskkill)...")
                        try:
                            # Usamos el mismo comando que en reinicio_seguro
                            subprocess.run(["taskkill", "/F", "/IM", "SCUMServer.exe"], 
                                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, 
                                         stdin=subprocess.DEVNULL, creationflags=0x08000000)
                        except: pass
                    time.sleep(1)
                
                # 3. Aplicar cambios (SOLO DESPUÉS DE ESTAR APAGADO)
                # Esto evita que el servidor sobrescriba el archivo al cerrarse.
                self.log(f">> Aplicando cambios de configuración para: {nombre_perfil}")
                self.editor.guardar_configuracion(contenido)
                
                time.sleep(1) # Breve pausa para asegurar I/O
        ...
        try:
            with open(self.tasks_file, 'r') as f:
                ...
                for t in lista:
                    ...
                    self.programar_tarea(t["hora"], t["perfil"], dias, task_id=t_id)
        except: pass
