...
class UserManager:
    ...
    def sincronizar_con_archivos(self):
        """Lee Ban.txt y Whitelist.txt e importa IDs que no estén en la DB"""
        self.importar_desde_txt("BAN", self.ban_file)
        self.importar_desde_txt("VIP", self.whitelist_file)

    def importar_desde_txt(self, tipo, archivo):
        if not os.path.exists(archivo): 
            self.log(f"⚠ No se encontró archivo {tipo} en: {archivo}")
            return
        ...
        try:
            ...
            cambios = False
            for steam_id in lineas:
                # Verificar si ya existe en la DB con ese tipo
                existe = any(u["id"] == steam_id and u["tipo"] == tipo for u in self.db)
                if not existe:
                    self.log(f"📥 Importando {tipo} existente: {steam_id}")
                    self.db.append({
                        "id": steam_id,
                        "tipo": tipo,
                        "expira": "PERMANENTE",
                        "notas": "Importado de archivo"
                    })
                    cambios = True
            ...
            self.log(f"❌ Error importando {tipo}: {e}")
